    ...
    protected int buildExcelTableHeader(int startRow) {
        ...
        for (x = 0; x < rowsetHeader.length; x++) {
            ...
            for (y = 0; y < maxColumns && y < rowsetHeader[x].length; y++) {
                ...
                // add the name of the dimensions for Yes | No header values
                if(currentHeader != null && (currentHeader.equals("Yes") || currentHeader.equals("No"))) {
                    currentHeader += " - " + rowsetHeader[x][y].getParentDimension();
                }
